                                        ...
                                        <div class="job-item p-4 mb-4">
                                            <div class="row g-4">
                                                <div class="col-sm-12 col-md-8 d-flex align-items-center">
                                                    ...
                                                    <a href="{{route('JobPortal.GetJobDetail',['job_id'=>$getJobsList->id])}}" ><img class="flex-shrink-0 img-fluid border rounded"
                                                    src="{{ asset('uploads/company_logo/'.$getJobsList->company_logo) }}" alt=""
                                                ...
                                                <div
                                                    ...
                                                    <input type="hidden" id="job{{$getJobsList->id}}" value="{{$getJobsList->id}}">
                                                    <input type="hidden" id="shiftId{{$getJobsList->id}}" value="{{$jobShift->id}}">
                                                    <input type="hidden" id="cat{{$getJobsList->id}}" value = "{{$getJobsList->job_category_id}}">

                                                    @if (Auth::user())
                                                        <div class="d-flex mb-3">
                                                            <a onclick="addWishList({{$getJobsList->id}})"  class=" btn btn-light btn-square me-3" href="#"><i
                                                    ...
                                                    <small class="text-truncate"><i
                                                            class="far fa-calendar-alt text-primary me-2"></i>Date Line: {{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $getJobsList->job_end_date)->format('Y-m-d') }}</small>
                                                </div>
                                            </div>
                                        </div>
                                    @endif
